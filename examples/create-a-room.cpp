 ...
 * ===============
 *
 * This is a libleet example which creates a basic room.
 */
#include <iostream>
#include <string>
#include <algorithm>
...
int main() {
    ...
    conf.Preset = LEET_PRESET_PUBLIC;
    conf.Name = "My super awesome room";
    conf.Alias = "my-new-super-cool-channel-xd-2";
    conf.Topic = "My super awesome topic";

    leet::Room::Room myNewRoom = leet::createRoom(&resp, &conf);

    if (checkError() == true) { // Yeah, appears something went wrong.
        return false;
    }

    std::cerr << "Name: " << myNewRoom.Name << "\n";
    std::cerr << "Topic: " << myNewRoom.Topic << "\n";
    std::cerr << "Alias: " << myNewRoom.Alias << "\n";
    std::cerr << "ID: " << myNewRoom.roomID << "\n";
}
